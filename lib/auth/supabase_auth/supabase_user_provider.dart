...
class KitraviaSupabaseUser extends BaseAuthUser {
  KitraviaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> kitraviaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = KitraviaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
